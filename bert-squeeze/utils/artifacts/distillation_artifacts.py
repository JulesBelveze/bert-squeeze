...
from ...models import LtCustomLabse, LtCustomBert
...
class DistillationArtifactsLoader:
    ...
    DISTIL_CLASSES = {
        "t2lr": Transformer2LR,
        "t2t": Transformer2Transformer
    }

    def __init__(self, config):
        self.config = config

        config.teacher.checkpoint_path = resource_filename("bert-squeeze", config.teacher.checkpoint_path)
        self.fine_tuned_teacher = load_model_from_exp(config.teacher.checkpoint_path, self.teacher_class)
    ...
    def student_featurizer(self):
        return {
            ...
            "transformer": TransformerFeaturizer(self.config.dataset, self.config.student.pretrained_tokenizer,
